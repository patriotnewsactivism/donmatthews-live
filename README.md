...
4. `https://donmatthews.live/health` returns `ok: true`, `platform: vercel`, and reports xAI configured.
5. The Vercel routes `/voice`, `/telnyx/voice`, `/twilio/voice`, `/stream`, and `/xai/sip` are present.

Once those conditions are met, the actual emergency cutover is only a carrier routing change: point the Telnyx voice webhook to `POST https://donmatthews.live/voice`, then place a verification call. Do not delete Cloud Run until the Vercel call path has been verified.
